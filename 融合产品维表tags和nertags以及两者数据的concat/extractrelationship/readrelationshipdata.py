...
@File:readrelationshipdata.py
@Time:2022/11/9 14:03
@ReadMe: 读取包含关系的数据; 也是确定本体 - 实体 - 关系的过程; 同时读取了Giikin商品的标签信息
"""
import os
import csv
import json
...
# todo 读取商品标签数据
basepath = '../nertags'  # part of nertags data demo  save to ./data-
# basepath = r'F:\allinoneisfast\giikinkgpart-ner\nertags'  # all nertags data； save to ./data
...
# todo 读取商品其他标签及属性数据
with open('../giikin_prtswithstandards_tags.pick', 'rb') as f:
    goodstags = pickle.load(f)
    # todo 先确定所有的标签的种类
    # alltagset = {}
    # alltagset['product_id'] = ''
...
# todo 读取产品信息维表
with open('../tb_dim_pro_gk_product_df/tb_dim_pro_gk_product_df.pick', 'rb') as f:
    prtclsinfo = pickle.load(f)
    # prtclsinfo_Values = prtclsinfo.values
