...
"""
@Author:UmeAI
@File:FB11月份Top优化师s他们投放的产品及其广告系列数据.py
@Time:2022/11/26 15:23
@Read: 该数据统计的基础思路：
        首先读取了各个家族的Top优化师s的ID数据；（这里有一个依赖是每个月各团队的Top优化师s可能会不同，因此要注意月份）
        然后读取了所有投放在FB平台的产品，匹配了这些优化师投放在FB平台的都有哪些产品（无月份依赖）；
        最后读取了这些Top优化师投放出来的广告系列都有哪些（无月份依赖）；
"""
import pickle
import pymysql
import pandas as pd
...
# todo 确定优化师投放的商品
# 202211月份 - 筛选特定家族的优化师
火凤凰Top优化师sID_11 = df.loc[df["family_name"] == "火凤凰家族"]['opt_id'].to_list()
神龙Top优化师sID_11 = df.loc[df["family_name"] == "神龙家族"]['opt_id'].to_list()
精灵Top优化师sID_11 = df.loc[df["family_name"] == "精灵家族"]['opt_id'].to_list()
红杉Top优化师sID_11 = df.loc[df["family_name"] == "红杉家族"]['opt_id'].to_list()  # 11月份相对没有其它家族好
金牛Top优化师sID_11 = df.loc[df["family_name"] == "金牛家族"]['opt_id'].to_list()  # 11月份相对没有其它家族好
金狮Top优化师sID_11 = df.loc[df["family_name"] == "金狮家族"]['opt_id'].to_list()
金蝉Top优化师sID_11 = df.loc[df["family_name"] == "金蝉家族"]['opt_id'].to_list()  # 11月份相对没有其它家族好
雪豹Top优化师sID_11 = df.loc[df["family_name"] == "雪豹家族"]['opt_id'].to_list()  # 11月份相对没有其它家族好
飞马Top优化师sID_11 = df.loc[df["family_name"] == "飞马家族"]['opt_id'].to_list()  # 11月份相对没有其它家族好

# todo 接下来做的是把好的优化师投放的商品选择了出来;
# wait 但是这里没有完成把当前月度的下优化师投放的商品删选出来，这里只是匹配到了这个优化师投放的所有商品
# 火凤凰团队11月份所有优化师投放出来的产品数据
火凤凰Top优化师sID_11_product = {}
for eachOptId in 火凤凰Top优化师sID_11:
    lineIndex = ader_idarea_id_groups.get(str(eachOptId))
